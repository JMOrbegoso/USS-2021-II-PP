...
string helpersClass::toLowerCase(string text) {
...
string helpersClass::toString(int number) { return to_string(number); }
string helpersClass::toString(float number) { return to_string(number); }

char* helpersClass::toCharArray(string text) {
  char* charArray;
  string str_obj(text);
  charArray = &str_obj[0];
  return charArray;
...
void helpersClass::pauseProcess() {
#if defined(WIN32) || defined(_WIN32) || \
    defined(__WIN32) && !defined(__CYGWIN__)
  system("pause");
#else
...
void helpersClass::addDelay(int sec) {
#if defined(WIN32) || defined(_WIN32) || \
    defined(__WIN32) && !defined(__CYGWIN__)
  Sleep(sec * 1000);
#else
...
void helpersClass::gotoxy(int x, int y) {
#if defined(WIN32) || defined(_WIN32) || \
    defined(__WIN32) && !defined(__CYGWIN__)
  HANDLE hcon;
...
string helpersClass::concatenateStrings(string acumulator, string toAdd,
                                        string separator) {
...
bool helpersClass::containsText(string textBase, string textToFind) {
...
string helpersClass::getGenre(char genre) {
  if (genre == 'm')
    return "Masculino";
  else if (genre == 'f')
    return "Femenino";
...
string helpersClass::requestText(string message, long unsigned int minLength) {
  string text;
...
char helpersClass::requestGenre(string message) {
  char genre;
...
int helpersClass::requestIntegerNumber(string requestMessage,
...
float helpersClass::requestFloatNumber(string requestMessage,
                                       string errorMessage, float min) {
  float amount;
...
float helpersClass::requestMoney(string message, float min) {
...
float helpersClass::requestMoney(string message, float min, float max) {
  string requestMessage, errorMessage;
...
string helpersClass::requestDate(string message) {
  string date;
...
bool helpersClass::requestBool(string message) {
  char boolValue;
