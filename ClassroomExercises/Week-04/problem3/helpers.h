...
class helpers {
 public:
  ...
  static string toString(int number) { return to_string(number); }
  static string toString(float number) { return to_string(number); }
  ...
  static bool containsText(string textBase, string textToFind) {
    if (toLowerCase(textBase).find(toLowerCase(textToFind), 0) !=
        string::npos) {
      return true;
    }
    return false;
  ...
  static string requestDate(string message) {
    ...
    int day = requestIntegerNumber(
        "Día del mes:",
        "Por favor ingrese un valor entre 1 y " + toString(maxDays), 1,
        maxDays);
